...
export default function ListOfDecks() {
    ...
    db.collection("decks").orderBy("createdAt", "desc").get()
        .then((documentSnapshots) => {
            const dbDecks = documentSnapshots.docs.map(doc => ({id: doc.id, ...doc.data()}));
...

            {
                decks.map(deck =>
                    <SingleDeckContainerStyle>
                        ...
                        <CardsContainerStyle>
                            {
                                deck.cards.map((card) => <Card card={card} isDeckCard showDeck/>
                                )
                            }
