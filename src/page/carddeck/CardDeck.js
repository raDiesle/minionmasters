...
export function CardDeck({
                             selectedCardEvent: {eventId: cardSelectedEventId, card: {iD: selectedCardId}, action},
    ...
    useEffect(() => {
        ...
        if (cardSelectedEventId !== 0 && prevSelectedEventId !== cardSelectedEventId && !isCardAlreadyOnSelectedSlot && lastSelectedCards[prevSelectedSlot]?.eventId === lastSelectedCards[currentSelectedSlot]?.eventId) {
            ...
            const isCardNotInDeckYet = typeof lastSelectedCards.find(({card: {iD}}) => selectedCardId === iD) === 'undefined';
            if (isCardNotInDeckYet) {
                ...
                const numberOfWildcards = lastSelectedCards.map(({count}) => count).reduce((total, current) => {
                ...
                if (isAllowedToAddAnotherWildcard) {
                    const positionOfExistingOccurence = lastSelectedCards.findIndex(({card: {iD}}) => selectedCardId === iD);
