...
export default function AgainstCards({
  cardModalId,
  againstKey,
  votedCards: votedCardiDs,
  triggerDataRefresh,
  handleEmptyCardDeckSlotClick,
  updateUpvoteSelection,
}) {
  ...
  const handleDownVote = (iD, idx) => {
    ...
    const newOrderedOrRemovedCards =
      idx === lastPosition
        ? votedCardiDs.filter((_, index) => index !== lastPosition)
        : swapArrayElements(votedCardiDs, idx, idx + 1);
        ...
        {votedCardsData.map((votedCardData, idx) => (
          <div key={votedCardData.iD}>
            <Card card={votedCardData}>
              ...
              <OrderOverlayStyle>{idx + 1}</OrderOverlayStyle>
            </Card>
            <VotingStyle>
              <SingleVoteStyle>
                <FontAwesomeIcon
